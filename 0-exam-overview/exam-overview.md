# CompTIA CySA+ Exam Overview

## Certification

**Certification:** CompTIA Cybersecurity Analyst (CySA+)

**Exam:** CS0-004

## Purpose of This Repository

This repository documents my learning journey while preparing for
the CompTIA CySA+ certification.

It combines:

- Exam preparation
- Technical notes
- Hands-on labs
- Security tools
- Threat detection
- Vulnerability management
- Incident response
- Threat intelligence
- Security operations

## Learning Approach

My preparation combines:

1. CompTIA CySA+ study materials
2. Technical notes
3. Hands-on labs
4. Practice questions
5. Flashcards
6. Scenario-based investigations
7. Revision

## Practical Focus

I will focus on developing practical skills in:

- Security monitoring
- Threat detection
- Log analysis
- SIEM
- Indicators of compromise (IOCs)
- Threat intelligence
- Threat hunting
- Vulnerability management
- Incident response
- Network analysis
- Security controls

## Progress

| Area | Status |
|---|---|
| Exam Overview | In Progress |
| Security Operations | Not Started |
| Vulnerability Management | Not Started |
| Incident Response | Not Started |
| Reporting & Communication | Not Started |
| Practical Labs | Not Started |
| Practice Questions | Not Started |
| Final Revision | Not Started |
| Exam | Not Attempted |

## Goal

Build practical cybersecurity analysis skills while preparing for
the CompTIA CySA+ certification.

---

**Learning → Practising → Investigating → Documenting → Improving**


---

## Exam Domain Breakdown

| Domain | Area | Weight | Status |
|---|---|---:|---|
| **Domain 1** | Security Operations | **34%** | 🔄 In Progress |
| **Domain 2** | Vulnerability Management | **26%** | ⏳ Not Started |
| **Domain 3** | Incident Response and Management | **24%** | ⏳ Not Started |
| **Domain 4** | Reporting and Communication | **16%** | ⏳ Not Started |
| | **Total** | **100%** | |

### Domain 1 — Security Operations
**34%**

Focus areas will include:

- Security monitoring
- Threat detection
- Security analytics
- Log analysis
- SIEM
- Indicators of Compromise (IOCs)
- Threat intelligence
- Threat hunting
- Security tools and technologies

### Domain 2 — Vulnerability Management
**26%**

Focus areas will include:

- Vulnerability identification
- Vulnerability scanning
- Vulnerability assessment
- Risk prioritisation
- Remediation
- Security controls

### Domain 3 — Incident Response and Management
**24%**

Focus areas will include:

- Incident detection
- Incident analysis
- Incident response
- Containment
- Eradication
- Recovery
- Evidence handling
- Post-incident activities

### Domain 4 — Reporting and Communication
**16%**

Focus areas will include:

- Security reporting
- Incident reporting
- Vulnerability reporting
- Risk communication
- Security metrics
- Stakeholder communication

---


## Test Your Knowledge

Before starting the detailed CySA+ learning journey, test your
current knowledge with this interactive pre-assessment.

### 🧠 CySA+ Knowledge Test

**[▶ Try the CySA+ Knowledge Test](https://supunkaru.github.io/CompTIA-CySA-CS0-004-/0-exam-overview/Pre_Quiz.html)**

This 20-question assessment provides:

- Immediate feedback
- Answer explanations
- Score tracking
- Progress tracking
- Final score

> This test is intended as a baseline assessment. I will retake it
> later to measure my improvement throughout my CySA+ journey.

---


